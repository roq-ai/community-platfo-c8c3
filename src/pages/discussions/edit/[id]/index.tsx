...
import { getDiscussionById, updateDiscussionById } from 'apiSdk/discussions';
...
function DiscussionEditPage() {
  ...
  const handleSubmit = async (values: DiscussionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDiscussionById(id, values);
      mutate(updated);
      resetForm();
